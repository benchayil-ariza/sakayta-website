...
(function () {
  ...
  function updateNavbar() {
    ...
    if (isAuthPage()) {
      // On authentication pages: ALWAYS show Home button, hide/show logout based on login state
      setElementVisible(".nav-login-btn", true);    // Always show Home button on auth pages
      setElementVisible("#nav-logout-btn", loggedIn); // Show logout only when logged in
    } else {
      // On public pages: show login/signup buttons when not logged in
